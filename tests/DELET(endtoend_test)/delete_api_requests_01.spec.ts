...
test('Create DELETE API Request in playwright', async ({ request }) => {
    ...
    expect(await deleteApiResponse.text()).toBe('Created');

    console.log('===GET API after delete====');

    //GET call after delete
    const getApiResponseAfterDelete = await request.get(`/booking/${bId}`);
    
    //Validate GET status code after delete
